...
function fillInspectionPage(
  ...
  if (checkerContent) {
    const sigImg = sig
      ? `<img src="${sig}" style="max-height:42px;max-width:100%;object-fit:contain;display:block;margin:0 auto 2px;" />`
      : '';
    (checkerContent as HTMLElement).innerHTML =
      `${sigImg}<div style="text-align:center;font-size:9px;font-weight:bold;">${escapeHtml(insp.inspector_name)}</div>`;
  }
}
